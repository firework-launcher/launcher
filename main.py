...
def run_pattern_threaded(pattern):
    ...
    with app.app_context():
        socketio.emit('running_pattern', pattern, broadcast=True)
    ...
    for pin in pins_changed:
        ...
        with app.app_context():
            socketio.emit('firework_launch', {'firework': pin[1], 'launcher': pin[0]}, broadcast=True)
    launcher_io.run_pattern(pattern_data)
    with app.app_context():
        socketio.emit("finished_pattern", pattern, broadcast=True)
